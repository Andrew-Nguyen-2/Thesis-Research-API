...
public class ProcessMessage {
	 ...
	 * 
	 * @return A ReceiveObj object containing the received filename and thread that
	 *         is running or null.
	 */
	public ReceiveObj process() {
		if (this.message != null) {
			Log.received(String.format(" [x] Received %s", this.message));
	 ...
	 * 
	 * @param forConvert True if the user is requesting the data from the user for
	 *                   converting. False otherwise.
	 */
	...
	private void wantData(boolean forConvert) {
		...
		for (FileData filedata : data) {
			...
			Boolean alreadyRequested = this.user.getFilesRequested(originSenderID) != null
					&& this.user.getFilesRequested(originSenderID).contains(filename);

			// if the format the sender sent is one the user wants and the user has not
			// requested the file
	...
	/**
	 * Helper function for wantData when a user who can translate receives a request
	 * for translation.
	 * 
	 * @return An array with a list of specific want formats, a list of the
	 *         announced data, the original message id, and the producer id.
	 */
	private Object[] getWantArgs() {
		...
		// check if the format the user is requesting the data to be translated to, is
		// valid for the translator
		for (String format : requestFormats) {
			for (Map.Entry<String, ArrayList<String>> entry : this.convertFormats.entrySet()) {
				...
				// if the format requested is a destination format, add the original format to
				// newWantedFormats
